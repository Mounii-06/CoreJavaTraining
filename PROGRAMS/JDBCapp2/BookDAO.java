...
public class BookDAO {
	public int addBook(BookBean obj) {
		...
		try {
			...
			String ins_str = "insert into book1 values(?,?,?,?)";
	...
	public ArrayList getBookDetailsById() throws Exception {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		con = BookDB.getConnection();
		String sel_str = "Select bookId,title,price from book1";
	...
	public int delrow(int bookId) {
		...
		try {
			con = BookDB.getConnection();
			String del_str = "delete from book1 where bookId=?";
